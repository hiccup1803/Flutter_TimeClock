part of 'detail_task_carousel_page_modal.dart';
      ...
      builder: (context, state) {
        ...
        return WillPopScope(
          onWillPop: () async {
            if (!loading) {
              if (changed) {
                Navigator.pop(context, DetailTaskCarouselPageModal.NOTE_CHANGED);
                return false;
              } else {
                return true;
              }
...
class OffTimeForm extends StatefulWidget {
  ...
  const OffTimeForm(this.initialNote);
    ...
    return BlocConsumer<EditAdminFileCubit, EditAdminFileState>(
      ...
      listener: (context, state) {
        if (state is EditAdminFileError) {
          ...
          Navigator.of(context).pop(DetailTaskCarouselPageModal.ADMIN_FILE_DELETED);
        } else if (state is EditAdminFileSaved) {
      ...
      builder: (context, state) {
        ...
        if (state is EditAdminFileReady) {
          adminSessionFile = state.adminTaskFile!;
          processing = false;
        } else if (state is EditAdminFileProcessing) {
          adminSessionFile = state.adminTaskFile!;
        } else if (state is EditAdminFileSaved) {
