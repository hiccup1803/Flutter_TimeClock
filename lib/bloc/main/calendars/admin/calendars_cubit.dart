...
class CalendarsCubit extends Cubit<CalendarsState> {
  CalendarsCubit(this._projectsRepository, this._historyRepository, this._sessionsRepository,
  ...
  final log = Logger('CalendarCubit');
  late DateTime _selected;
  List<bool> _filterList = [true, false, false];
  int assignee = 0;
  ...
  Future<void> init(DateTime initialDay) async {
    ...
    await _projectsRepository.getAdminTasks(first, last, assignee).then((value) {
      _tasks = value;
    }, onError: (e, stack) {});

    await refreshOffTime(assignee);
    await refreshSession(assignee);
  }

  void changeMonth(DateTime date, {bool force = true}) {
    // _projectsRepository.getTasks();
  ...
  Future<void> refreshSession(int assignee) async {
    ...
    await _sessionsRepository.getAdminCalendarItems(first, last, 1, assignee).then(
      (paginated) {
  ...
  Future<void> refreshOffTime(int assignee) async {
    final key = _selectedKey;
    final first = _selected.firstDayOfMonth;
    final last = _selected.nextMonth.firstDayOfMonth;
    ...
    await _historyRepository.getAdminCalendarItems(first, last, 1, assignee).then(
      (paginated) {
        Loadable<Paginated> loadable;
        if (paginated.hasMore) {
          loadable = Loadable.inProgress(paginated);
  ...
  Future<bool> updateFilter(List<bool> value, {int? assignee}) async {
    _filterList = value;
    this.assignee = assignee ?? 0;
